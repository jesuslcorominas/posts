...
fun DbPost.toDomainPost() =
    DomainPost(id, userId, title, body, author = null, comments = ArrayList())
...
fun DomainPost.toDbPostDetail() = PostDetail(
    DbPost(id, userId, title, body),
    author?.toDbAuthor(),
    comments.map { it.toDbComment() })
